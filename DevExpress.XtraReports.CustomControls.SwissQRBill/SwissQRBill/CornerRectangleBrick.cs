...
namespace DevExpress.XtraReports.CustomControls.SwissQRBill {
    ...
    public class CornerRectangleBrickExporter : VisualBrickExporter {
        ...
        public void DrawCornedRectangle(IGraphics gr, RectangleF rect) {
            float markSize = GraphicsUnitConverter.Convert(Constants.CornerMarkSize, GraphicsDpi.Millimeter, GraphicsDpi.UnitToDpi((GraphicsUnit)gr.PageUnit));
            float markThickness = GraphicsUnitConverter.Convert(Constants.CornerMarkThickness, GraphicsDpi.Point, GraphicsDpi.UnitToDpi((GraphicsUnit)gr.PageUnit));
